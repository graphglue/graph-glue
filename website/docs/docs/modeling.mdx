...
Inheriting from interfaces is possible, however nonr of the GraphGlue annotations (`@DomainNode`, `@Authorization`, `@AdditionalFilter`, `@AdditionalOrder`) is supported there, however, it can e.g. be used to share common `@GraphQLDescriptions`.
