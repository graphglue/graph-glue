...
1. Property which is exposed in the GraphQL schema. As it is annotated with both `@OrderProperty` and `@FilterProperty`, connections of type `Tree` can both be filtered and ordered by this property.
